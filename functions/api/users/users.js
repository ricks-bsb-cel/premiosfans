...
const cartosCalls = require('../cartos/cartos.calls');
const cartosUsers = require('../cartos/cartos.users');
...
const collectionAppUser = firestoreDAL.appUsers();
...
exports.updateUserProfile = (request, response) => {
    ...
    return getUserInfoWithToken(token)
        ...
        .then(_ => {
            return Promise.all([
                appUsers.setAppUser(userData.uid, profileData),
                cartosUsers.publishCartosUserResponseById(userData.uid)
            ]);
        })
...
exports.cartos = {
    users: {
        user: (request, response) => {
            ...
            const
                token = global.getUserTokenFromRequest(request, response),
                Users = new cartosCalls.Users();

            if (!token) { return response.status(400).end(); }
            ...
            return getUserInfoWithToken(token)
                ...
                .then(result => {
                    ...
                    if (result.tokenSource !== 'firebase') { throw global.newError('Token inválido para esta operação', 401); }

                    userTokenData = result.data;

                    if (!userTokenData.cpf) { throw global.newError('CPF Inválido', 401); }

                    return Users.getUsers(userTokenData.cpf);
                })
                ...
                .then(result => {
                    ...
                    result = {
                        data: {
                            appUser: userTokenData,
                            cartosUser: result
                        }
                    };

                    result.data.cartosUser.exists =
                        typeof result.data.cartosUser.data.emailValidated === 'boolean' &&
                        typeof result.data.cartosUser.data.phoneValidated === 'boolean';
        ...
        register: (request, response) => {
            ...
            const token = global.getUserTokenFromRequest(request, response);
            const cartos = require('../cartos/cartos');
            ...
            return getUserInfoWithToken(token)
                ...
                .then(user => {
                    return cartos.createUser(user.data.uid);
                })
                ...
                .then(result => {
                    ...
                    return response.status(200).json(
                        global.defaultResult({ data: data }, true)
                    );
        ...
        sendConfirmationCodeToEmail: (request, response) => {
            const token = global.getUserTokenFromRequest(request, response);
            const cartos = require('../cartos/cartos');
            ...
            return getUserInfoWithToken(token)
                ...
                .then(user => {
                    return cartos.sendConfirmationCodeToEmail(user.data.uid);
                })
        ...
        confirmEmailCode: (request, response) => {
            const token = global.getUserTokenFromRequest(request, response);
            const code = request.body.code || null;
            const cartos = require('../cartos/cartos');

            if (!token || !code || code.length < 4 || code.length > 12) {
            ...
            return getUserInfoWithToken(token)
                ...
                .then(user => {
                    return cartos.confirmEmailCode(user.data.uid, code);
                })
                ...
                .then(result => {
                    ...
                    else if (result.result && result.result.code === "COG-001") {
...
exports.account = {
    ...
    checkAccountData: (request, response) => {

        const token = global.getUserTokenFromRequest(request, response);
        const cartos = require('../cartos/cartos');
        const type = request.params.type;

        if (!token || !type) { return response.status(400).end(); }

        return getUserInfoWithToken(token)

            .then(result => {
                return cartos.checkAccountData(result.data.uid, type);
            })
    ...
    checkUser: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            ...
            .then(userData => {
                ...
                if (!userData.data.cpf) { throw new Erro('O usuário atual não está registrado com nenhum CPF'); }

                return Promise.all([
                    cartos.checkUser(userData.data.uid),
                    admin.database().ref(`/app/${userData.data.uid}/result-send-confirmation-code-to-email`).once("value")
                ])
            ...
            .then(promiseResult => {
                ...
                const lastSendConfirmationEmail = promiseResult[1].val() || {};

                result.userExists = result.info.userExists;

                // Contas já aberta TEM peopleId e 
                result.accountPFExists = typeof result.accounts.pf.peopleId === 'string';
                result.accountPJExists = typeof result.accounts.pj.peopleId === 'string';

                result.emailValidated = result.info.emailValidated;
                result.phoneValidated = result.info.phoneValidated;

                result.emailCodeSent = Boolean(lastSendConfirmationEmail.dtEnvio);

                result.emailCodeCanSend = typeof lastSendConfirmationEmail.expire === 'undefined' ? true : lastSendConfirmationEmail.expire < nowMilliseconds;
    ...
    getUserAccountToken: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            .then(user => {
                return cartos.getUserAccountToken(user.data.uid);
            })
    ...
    statusConta: (request, response) => {
        ...
        if (!token || !type) { return response.status(400).end(); }
        if (type !== 'pj' && type !== 'pf') { return response.status(400).end(); }

        return getUserInfoWithToken(token)

            .then(user => {
                return cartos.accountStatus(user.data.uid, type);
            })
    ...
    statusDetails: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            ...
            .then(user => {
                return cartos.accountStatusDetails(user.data.uid, type);
            })
    ...
    detalhesConta: (request, response) => {
        ...
        if (!token) { return response.status(400).end(); }
        if (type !== 'pf' && type !== 'pj') { return response.status(400).end(); }

        return getUserInfoWithToken(token)

            .then(user => {
                return cartos.detalhesConta(user.data.uid, type);
            })
    ...
    abrirContaPessoaFisica: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            ...
            .then(user => {
                return cartos.abrirContaPessoaFisica(user.data.uid);
            })
    ...
    abrirContaPessoaJuridica: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            ...
            .then(user => {
                return cartos.abrirContaPessoaJuridica(user.data.uid);
            })
    ...
    enviarDocumentos: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            ...
            .then(user => {
                return cartos.enviarDocumentos(user.data.uid, type);
            })
    ...
    checkZoepayCompany: (request, response) => {
        ...
        return getUserInfoWithToken(token)
            ...
            .then(user => {
                return cartos.checkZoepayCompany(user.data.uid, type);
            })
    ...
    /*
    refreshTransactions: (request, response) => {
        ...
        if (token) { // Chamada via token é a chamada principal, e traz sempre a primeira página de consultas

            return getUserInfoWithToken(token)

                .then(user => {
                    return cartos.refreshTransactions(uid, 1);
                })

                .then(result => {
                    return response.status(200).json(
                        global.defaultResult({ rows: result }, true)
                    );
                })
        ...
        } else if (uid) { // A chamada direta com UID é utilizada internamente para processar as próximas páginas

            return admin.database().ref(`/app/${uid}user`).once("value")
                .then(userData => {

                    userData = userData.val() || null;

                    if (!userData) { throw newError('Requisição inválida', 400); }

                    return cartos.refreshTransactions(uid, page);
                })
    ...
    saveTransaction: (request, response) => {
        ...
        try {
            ...
            if (!request.body) {
                throw newError('Invalid payload', 500);
            }

            let data = global.base64ToJson(request.body.message.data);

            data._dtReceived = admin.firestore.Timestamp.now();
            data._host = global.getHost(request)

            data._messageId = request.body.message.messageId;
            data._publishTime = request.body.message.publishTime;

            data = Object.assign(data, request.body.message.attributes);

            const cartos = require('../cartos/cartos');

            return cartos.addTransaction(data)
    ...
    */
    ...
    getPixKeys: (request, response) => {

        const cartos = require('../cartos/cartos');

        const token = global.getUserTokenFromRequest(request, response);
        const idConta = request.params.idConta || null;
        ...
        return getUserInfoWithToken(token)
            ...
            .then(getUserInfoWithTokenResult => {
                ...
                userInfo = getUserInfoWithTokenResult;

                if (!userInfo.data.idEmpresa) {
                    throw global.newError(`O usuário ${userInfo.data.uid} não está selecionado em nenhuma empresa.`);
                }

                return cartos.getPixKeys(userInfo.data.idEmpresa);
            })

            .then(result => {
                return response.status(200).json(
                    global.defaultResult({
                        idEmpresa: userInfo.data.idEmpresa,
                        data: result
                    })
                );
            })

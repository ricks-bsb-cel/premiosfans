        ...
        return admin.database().ref(path).once("value")
            .then(tokenData => {
                ...
                if (
                    ...
                    if (tokenData.expire && nowMilliseconds < tokenData.expire) {
                        // O token de login existe e não está expirado. Retorna o Buffer.
                        result = tokenData;

                        return null;
                    } else {
                        // O token de login existe, mas está expirado. Tenta renova-lo
                        return cartosHttpRequest.login(cpf, password);
                    }
                }

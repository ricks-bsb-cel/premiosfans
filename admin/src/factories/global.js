...
let ngModule = angular.module('factories.global', [])
	...
	.factory('globalFactory',
		function (
			...
			const rgbAlpha = function (rgb) {
				rgb = rgb.replace(/[^\d,.]+/g, '').split(',');
				return rgb.length == 4 ? parseFloat(rgb[3]) : 1;
			}
			...
			const getSearchParams = function () {
				...
				params.forEach(function (value, key) { result[key] = value; });

				return result;
			}

			const trustAsHtml = function (html) {
			...
			const removeDiacritics = function (str) {
				if (!str) { return null; }
				if (!defaultDiacriticsRemovalMapChanges) {
					defaultDiacriticsRemovalMapChanges = defaultDiacriticsRemovalMap;
				}
				for (var i = 0; i < defaultDiacriticsRemovalMapChanges.length; i++) {
					str = str.replace(defaultDiacriticsRemovalMapChanges[i].letters, defaultDiacriticsRemovalMapChanges[i].base);
				}
				return str.toLowerCase();
			}

			const firstOrNull = function (arrayValue) {
			...
			const generateKeywords = function (v1, v2, v3, v4, v5, v6, v7) {
			...
			const isCPFValido = function (cpf) {
			...
			const isCNPJValido = function (cnpj) {
				...
				for (let i = 0; i <= 9; i++) {
					invalid.push(i.toString().repeat(14));
				}

				if (cnpj.length != 14 | invalid.includes(cnpj)) {
					return false;
				}
			...
			const capitalize = value => {
				...
				const artigos = ['e', 'ou', 'o', 'os', 'a', 'as', 'um', 'uns', 'uma', 'umas', 'a', 'ao', 'aos', 'à', 'às', 'de', 'do', 'dos', 'da', 'das', 'em', 'no', 'nos', 'na', 'nas'];
				let result = '';
			...
			*/
			...
			const sortArray = (objList, field, desc) => {
				var result = [];

				if (typeof _ === "undefined") {
					result = objList.sort(function (a, b) {
						if (a[field] > b[field]) return (desc ? -1 : 1);
						if (b[field] > a[field]) return (desc ? 1 : -1);
						return 0;
					})
				} else {
					result = _.orderBy(objList, [field], desc ? ["desc"] : ["asc"]);
				}

				return result;
			}

			const replace = (target, search, replacement) => {
			...
			const isValidDtNascimento = (unixValue, idadeMinima, idadeMaxima) => {
